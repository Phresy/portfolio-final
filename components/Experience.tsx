      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row items-end justify-between mb-20 gap-4">
          <div className="space-y-2">
            <h2 className="text-5xl md:text-7xl font-black tracking-tighter text-slate-900 dark:text-slate-100 uppercase italic">
              {lang === "zh" ? "历程" : "Timeline"}
            </h2>
            <p className="text-blue-600 dark:text-blue-400 font-bold tracking-[0.3em] uppercase text-xs ml-1">
              {lang === "zh" ? "职业足迹" : "Professional Footprint"}
            </p>
          </div>
          <div className="h-0.5 grow bg-slate-200 dark:bg-slate-800 mx-8 hidden md:block mb-4" />
        </div>
              ...
              <div className="md:col-span-10 relative bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 p-8 md:p-12 rounded-[3rem] transition-all hover:border-blue-500/30 hover:shadow-[0_20px_80px_rgba(37,99,235,0.1)]">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                  <div>
                    ...
                    <p className="text-blue-600 dark:text-blue-400 font-semibold text-lg">@{item.company}</p>
                  </div>
                  <div className="px-4 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-[10px] font-black uppercase tracking-widest w-fit border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100">
                    {item.type}
                  </div>
                </div>
